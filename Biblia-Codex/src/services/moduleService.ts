...
const detectCategory = (fileName: string): ModuleCategory => {
  ...
  if (name.includes('mapgeodata') || name.includes('.map.') || name.includes('.atlas.') || name.includes(' mapa') || name.includes(' atlas') || name.includes('mapa.') || name.includes('atlas.')) return 'map';
  ...
  for (const directory of [Directory.Data, Directory.Documents] as const) {
    try {
      ...
      try {
        ...
        if (!errorMsg.includes('exists') && !errorMsg.includes('eexist') && !errorMsg.includes('already')) {
          throw mkdirError;
        }
      }
